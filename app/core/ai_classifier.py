...
- 【NTT電話料金払込書】「NTT東日本電話料金等領収書振替払込書」「NTT西日本」の払込票では: ①取引先は「NTT東日本」または「NTT西日本」を正確に区別する（「東」と「西」は異なる会社。「NTT海日本」「NTT果日本」等の誤読は絶対禁止）。領収書に「東日本」と印字されていれば「NTT東日本」、「西日本」と印字されていれば「NTT西日本」とする。②金額は「うち消費税等相当額」から逆算して特定する（消費税相当額×11≒税込合計。例: 消費税相当額¥358なら税込≒¥3,938）。口座番号「00140-7-900500」・バーコード・電話番号の数字は金額として絶対に使わない。③発生日は「支払期日」を使う（「年月分」は請求対象月なので使わない）。"""
