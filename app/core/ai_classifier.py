...
以下のJSON形式のみで回答してください（前後の説明・コードブロック不要）:
{
  ...
  "total_amount": 実際に支払った合計金額（整数。課税・非課税を問わず全品目の合計＝「合計」欄の金額。例：課税計¥140＋非課税計¥7,550＝合計¥7,690→7690。優先順位: ①「現金入金額」②「ご請求額」「払込金額」「請求額」③「合計」「税込合計」。「課税計」「課税小計」「税抜合計」は合計ではないので使わない。クーポン・割引券がある場合は差引後の金額を使う。「お預り」「おつり」「ご返金額」は絶対に含めない）,
  ...
  "non_taxable_amount": 非課税・免税の合計金額（整数。EMS・国際郵便・切手等の非課税項目がある場合のみ。なければ0。例: 非課税計¥7,550→7550）,
  ...
  "purpose": "用途メモ（原則空文字。ただし領収書に管理番号・シリアル番号・製品番号・整理番号等の固有識別子がある場合のみ記入。例: Vプリカ2万の場合「管理番号:58795119734339」、ギフト券の場合「券番号:XXXX」）",
  "reason": "判定理由（一言）"
}
...
旅費交通費 / 通信費 / 水道光熱費 / 接待交際費 / 会議費 / 消耗品費 / 広告宣伝費 / 地代家賃 / 租税公課 / 社会保険料 / 外注費 / 福利厚生費 / 修繕費 / 預り金 / 諸雑費 / 雑損失
...
- 【取引先の特定ルール】領収書・レシートに記載された「発行元の店舗・事業者名」を使う。デパート・ショッピングモール・駅ビル・テナントビル内の飲食店・小売店の場合は、建物名・親会社名（東武鉄道・JR・東急・イオン等）ではなく、実際に営業している店舗名（レストラン名・チェーン名等）を使う。例: 東武百貨店内の飲食店 → その飲食店名（デニーズ、コメダ珈琲 等）。ただし東武百貨店・東急百貨店・高島屋等の百貨店自体での購入は百貨店名を使う。
...
- 【税金・社会保険料の納付は不課税】所得税・法人税・消費税・印紙税・固定資産税・住民税・事業税等の国税・地方税の納付、および社会保険料・厚生年金・健康保険料の納付は消費税の課税対象外（不課税）。taxable_10_amount=0, tax_10_amount=0, taxable_8_amount=0, tax_8_amount=0 とすること。取引先が「国税」「税務署」「都税」「市税」「県税」「府税」「年金機構」「協会けんぽ」「健康保険組合」等の行政機関・公的機関の場合も同様に全額不課税。
- 飲料・酒類（ビール・日本酒・ウイスキー等）が含まれる場合は beverage_amount に合計金額を記入
- 「お預り」「おつり」「釣銭」の金額は total_amount に絶対に使わない
- 【非課税・免税項目を含む領収書】EMS・国際郵便・切手等の非課税項目がある場合: total_amountは課税計+非課税計を合算した「合計」欄の金額を使う（例: 課税計¥140 + 非課税計¥7,550 = 合計¥7,690 → total_amount=7690）。taxable_10_amountには課税分のみ（¥140÷1.1=¥128）を記入。非課税分はtaxable_10_amountに含めない。
- 【郵便振替払込票・公共料金領収書】「電気料金等郵便振替払込金受領証」等の払込票では: ①取引先は「支払先（受取人）」の会社名を使う。払込人（支払う側）の名前・電話番号・口座番号・バーコードの数字は取引先として絶対に使わない。電気代なら東京電力・関西電力・中部電力等、ガスなら東京ガス・大阪ガス等が取引先。②払込金額の各桁が独立したボックスに印字されている（例: ¥ 4 3 3 は433円であり4333円ではない）。桁数は「うち消費税相当額」から逆算して確認すること（消費税39円なら税込約433円）。③発生日の優先順位: 「日附印」スタンプの日付（読み取れる場合）→ 「お支払期限日」（読み取れない場合）。「年月分」（例: 7-12）は請求対象月であり発生日に使わない。
- 【NTT電話料金払込書】「NTT東日本電話料金等領収書振替払込書」「NTT西日本」の払込票では: ①取引先は「NTT東日本」または「NTT西日本」を正確に区別する（領収書に「東日本」と印字されていれば「NTT東日本」、「西日本」と印字されていれば「NTT西日本」とする）。②金額は「うち消費税等相当額」から逆算して特定する（消費税相当額×11≒税込合計。例: 消費税相当額¥358なら税込≒¥3,938）。口座番号・バーコード・電話番号の数字は金額として絶対に使わない。③発生日は「支払期日」を使う（「年月分」は請求対象月なので使わない）。

【最終確認: 以下のルールは他の判断より優先する】
1. 取引先の特定: 領収書を発行した「店舗・事業者」の名前を使う。ビル名・百貨店名・鉄道会社名（東武・東急・JR・小田急等）は建物の所有者であり取引先ではない。テナント店舗の名前（飲食店名・小売店名）を取引先とする。ただし百貨店・スーパー自体で購入した場合はその店名を使う。
2. ガソリンスタンド: apollostation・ENEOS・エネオス・出光・昭和シェル・コスモ石油・JA-SS等のガソリンスタンドは、購入品目に関わらず必ず debit_account=旅費交通費, debit_subsidiary=ガソリン代 とする。
3. 飲食店: レストラン・カフェ・居酒屋・ファミレス等の飲食店は debit_account=会議費（社内打合せ・会食）または接待交際費（取引先接待）とする。消耗品費・諸雑費は使わない。"""
