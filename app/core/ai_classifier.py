...
- 【郵便振替払込票・公共料金領収書】「電気料金等郵便振替払込金受領証」等の払込票では: ①払込金額の各桁が独立したボックスに印字されている（例: ¥ 4 3 3 は433円であり4333円ではない）。桁数は「うち消費税相当額」から逆算して確認すること（消費税39円なら税込約433円）。②発生日は「日附印」（スタンプの日付）を使う。「お支払期限日」や「年月分」は発生日に使わない。"""
