...
以下のJSON形式のみで回答してください（前後の説明・コードブロック不要）:
{
  ...
  "taxable_10_amount": 税率10%の税抜本体金額（整数。領収書に「消費税額」「消費税10%」が明示されている場合はその値を使い、total_amountから再計算しないこと。明示なければ total_amount ÷ 1.1 を四捨五入）,
  "tax_10_amount": 消費税10%の金額（整数。領収書に明示された「消費税額」「消費税10%」の値を使う。クーポン差引後のtotal_amountから逆算しないこと）,
...
- クーポン・割引がある場合の total_amount: 「合計領収額」ではなく差引後の「現金入金額」または「ご請求額」を使う
