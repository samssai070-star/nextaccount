...
- 【NTT電話料金払込書】「NTT東日本電話料金等領収書振替払込書」「NTT西日本」の払込票では: ①取引先は必ず「NTT東日本」または「NTT西日本」（「NTT海日本」「NTT果日本」等の誤読は絶対禁止）。②金額は「¥X,XXX」と明記された払込金額のみを使う（口座番号「00140-7-900500」やバーコード番号と絶対に混同しない）。③発生日は「支払期日」を使う（「年月分」は請求対象月なので使わない）。"""
